...
const uiController = {
  ...
  addTransaction: function () {
    ...
    if (transactionService.addTransaction(transaction)) {
      ...
      this.showAlert(
        `Transação ${
          transaction.tipo === "entrada" ? "de entrada" : "de saída"
  ...
  updateSummaryCards: function () {
    ...
    if (saldo) {
      ...
      saldo.className = `card-text fs-4 fw-bold ${
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const savedTheme = localStorage.getItem("keihi_theme");
  if (savedTheme === "light") {
  ...
  themeToggle.addEventListener("click", () => {
    ...
    if (isDark) {
      body.classList.remove("dark-mode");
      body.classList.add("light-mode");
      localStorage.setItem("keihi_theme", "light");
    } else {
      body.classList.remove("light-mode");
      body.classList.add("dark-mode");
      localStorage.setItem("keihi_theme", "dark");
    }
  });
});
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (themeToggle) {
    themeToggle.addEventListener("click", () => {
      ...
      localStorage.setItem("keihi_theme", newTheme);
